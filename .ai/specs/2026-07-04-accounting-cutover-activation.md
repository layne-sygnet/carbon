...
- 2026-07-24: Implemented (PR `feature/accounting-cutover`, #1057). Migration `20260724120000_accounting-activation.sql` (enum value, `company` activation columns, `check_accounting_config_locked` triggers, grandfather backfill); service `getActivationReadiness`/`buildOpeningBalanceProposal`/`importOpeningTrialBalance`/`validateOpeningBalance`/`activateAccounting`/`closePreCutoverPeriods`; models validators + types; routes `x+/accounting+/activation.tsx` (+ `activation.import.tsx` CSV TB path); wizard UI; `seed-company` GA block behind `SEED_ACCOUNTING_ACTIVE`. Flag retirement (§5) remains a separate PR per Design Decision 7. Audit trail is via the immutable activation stamps + posted opening journal + per-period `closedBy/closedAt` (column stamping, per Design Decision 3); wiring an explicit event-system audit-log row is a follow-up.
